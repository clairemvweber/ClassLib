...
class LookupActivity : AppCompatActivity() {
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        Log.i("ISBN ACTIVITY", "request: " + requestCode.toString() + "result:" +resultCode.toString())
        if(requestCode == BARCODE_REQUEST){
           if( resultCode == RESULT_OK) {
            ...
            Toast.makeText(this,"ISBN not in database. Please enter book details manually.",Toast.LENGTH_LONG).show()
        }
        else {
            setResult(resultCode,data)
            finish()
